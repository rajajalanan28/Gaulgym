      ...
      <main className={`flex-1 ${user ? 'pt-[32px]' : 'pt-[120px]'} pb-[96px]`}>
        <div className="max-w-[800px] mx-auto px-6">
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mt-[32px] text-center">
            {[
              { val: '50+', label: 'Kelas Per Minggu' },
              { val: '24/7', label: 'Akses Member' },
              { val: '30+', label: 'Personal Trainer' },
              { val: '10k', label: 'Member Aktif' },
            ].map((s) => (
              <div key={s.label} className="bg-[var(--color-surface-1)] hairline-border p-[24px] rounded-[12px]">
                <div className="text-[32px] font-semibold text-[var(--color-ink)] mb-1 tracking-[-0.02em]">{s.val}</div>
                <div className="text-[13px] font-medium text-[var(--color-ink-subtle)]">{s.label}</div>
              </div>
            ))}
          </div>
        </div>
